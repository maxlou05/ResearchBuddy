...
export default function SearchBar() {
  ...
  const topicClick = async (e, topic) => {
    setQuery(topic)
    await searchTopics(e)
  }

  const arrowClick = async (topic) => {
    ...
    <>
      <div className="flex flex-col space-between">
        ...
        <div className="flex flex-row">
          ...
          <div>
            {results.map((topic) => (
              <SubTopic title={topic} onTitleClick={(e) => topicClick(e, topic)} onArrowClick={() => arrowClick(topic)}/>
            ))}
          </div>
        </div>
      </div>
