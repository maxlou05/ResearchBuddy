...
const copyInnerHTMLRecursive = (element) => {
    ...
    if (element.children && element.children.length > 0) {
      for (const child of element.children) {
        const childText = copyInnerHTMLRecursive(child)
        if (!copiedHTML.includes(childText)) copiedHTML += childText
        break
      }
    } else {
...
export default function Page() {
  ...
  const copyClick = () => {
    if (citationRef.current) {
      const copiedContent = copyInnerHTMLRecursive(citationRef.current)
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-24">
      <p className="text-4xl text-gray-900 dark:text-white" style={{ paddingBottom: '1.5rem' }}>Citations</p>
      <div>
        {loading && 
        <div role="status">
